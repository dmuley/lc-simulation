...
def generateRadiiThetas(n, circles, *args):
	...
	r = np.unique(np.append(np.unique(r), np.unique(r + np.pi)));
...
def rd2(coords, circles,opt = 0):
		...
		if opt == 2:
			u = [np.concatenate((np.unique(oh[a][oh[a] == 0]), oh[a][(oh[a] > 0.) & (oh[a] < oh[a].max() - 0.0001)], np.unique(oh[a][oh[a] == oh[a].max()]))) for a in range(0,len(coords[1]))];
			return u, coords[1];
				
def groupAndIntegrate(bounds, function = 1):
	rad = [np.sort(a[a >= 0.]) for a in bounds[0] if len(a) > 1];
	theta = np.array([bounds[1][p] for p in range(0,len(bounds[1])) if len(bounds[0][p]) > 1]);
	
	print len(rad), len(theta);
	...
	for i in range(0,len(d_theta)):
		h = 0;
		if len(rad[i]) % 2 == 0 and len(rad[i+1]) % 2 == 0:	
...
u = time.time();
c = [[5., 0., 0.],[1,3.5,0]]; #no need for dummy here
y = getTangentsIntersections(c);
m = generateRadiiThetas(20,y[0], y[1], y[2]);
f = rd2(m, c, opt = 1);
oh= groupAndIntegrate(f);
print "Integral = " + str(oh);
print time.time() - u;
